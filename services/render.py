...
def render_project_image(
    *,
    ...
    try:
        _report(progress_callback, "Rendering export...", 0, max(1, len(visible_imdb_ids)))
        ...
        for index, (imdb_title_id, cell) in enumerate(
            ...
            _report(
                progress_callback,
                f"Rendering export {index} / {total}: {label}",
                index - 1,
                total,
            )
